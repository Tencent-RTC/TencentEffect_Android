...
(function () {
    ...
    template.uniformJson = {
        "uniformMap": {
            "green_seg_chromakey": {
                "green_params": [0.35, 1.0, 1.0, 1.0], // [similarity, deshadow, corrosion,color]
                "tex_rect": [0.0, 0.0, 720.0, 1280.0],
                "average_green": [0.0, 1.0, 0.0, 0.0]
            },
            "Corrosion": {
                "tex_rect": [0.0, 0.0, 720.0, 1280.0],
                "corrosion": [1.0, 1.0, 0.0, 0.0] // [corrosion,color]
            },
            "green_seg_merge": {
                "tex_rect": [0.0, 0.0, 720.0, 1280.0],
                "merge_params": [1.0, 0.1, 0.08, 0.7], // [corrosion, despill, smooth, blendWeight]
                "frame_params": [0.0, 0.0, 0.0, 0.0], // [isFirstframe, ]
...
//    // 订阅InputEvent事件
    template.onInputEvent = function(params) {
        ...
        if (GreenSeg_jsonDataParams) {
            ...
            template.uniformJson.uniformMap.green_seg_chromakey.green_params[3] = color_value;
            template.uniformJson.uniformMap.Corrosion.corrosion[0] = corrosion_value; // corrosion
            template.uniformJson.uniformMap.Corrosion.corrosion[1] = color_value; // color
            template.uniformJson.uniformMap.green_seg_merge.merge_params[0] = corrosion_value; // corrosion
            template.uniformJson.uniformMap.green_seg_merge.merge_params[1] = despill_value; // despill
            template.uniformJson.uniformMap.green_seg_merge.merge_params[2] = smooth_value; // smooth          
        }
        // 绿幕保护区域
        ...
        if (Merge_jsonDataProtectRect) {
            ...
            template.uniformJson.uniformMap.green_seg_merge.tex_protect_rect[0] = Merge_jsonDataProtectRect["tex_protect_rect"][0];
            template.uniformJson.uniformMap.green_seg_merge.tex_protect_rect[1] = 1.0 - Merge_jsonDataProtectRect["tex_protect_rect"][1];
            template.uniformJson.uniformMap.green_seg_merge.tex_protect_rect[2] = Merge_jsonDataProtectRect["tex_protect_rect"][2];
            template.uniformJson.uniformMap.green_seg_merge.tex_protect_rect[3] = 1.0 - Merge_jsonDataProtectRect["tex_protect_rect"][3];
        }
    }
    ...
    template.onFrameUpdate = function (currentTime, entityManager, eventManager) {
        ...
        template.uniformJson.uniformMap.green_seg_merge.tex_rect[2] = surfaceWidth;
        template.uniformJson.uniformMap.green_seg_merge.tex_rect[3] = surfaceHeight;
