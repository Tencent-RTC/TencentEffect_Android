 ...
 * 原子能力view
 */
public class TECapabilitiesPanelView extends FrameLayout implements View.OnClickListener, CompoundButton.OnCheckedChangeListener {
    ...
    private void initViews(Context context) {
        foldedLayout = (ConstraintLayout) LayoutInflater.from(context).inflate(R.layout.te_beauty_capabilities_panel_view_folded_layout, this, false);
        expandLayout = (ConstraintLayout) LayoutInflater.from(context).inflate(R.layout.te_beauty_capabilities_panel_view_expand_layout, this, false);
    ...
    public void onClick(View v) {
        ...
        } else if (v.getId() == R.id.te_capabilities_folded_view_camera_btn || v.getId() == R.id.te_capabilities_expand_view_camera_btn) {
            onCameraClick();
        }
    }
    ...
    private void initRadioGroup() {
        ...
        if (capabilitiesItems.size() > 3) {   //当item 大于三个时  左对齐，否则居中对齐
        ...
        for (int menuIndex = 0; menuIndex < capabilitiesItems.size(); menuIndex++) {
            ...
            btn.setTextSize(16);
            btn.setLines(1);
            btn.setTextColor(CustomDrawableUtils.createRadioGroupColorStateList(uiConfig.textCheckedColor, uiConfig.textColor));
