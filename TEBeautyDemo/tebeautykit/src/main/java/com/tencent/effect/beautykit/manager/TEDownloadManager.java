...
public class TEDownloadManager {
    ...
    public void download(TEMotionDLModel model, boolean isUnZip, TEDownloadListener downloadListener) {
        ...
        if (file.exists()) {
            ...
            // There is a scenario: during the decompression process, the process is killed, resulting in an incomplete extracted folder.
            // Therefore, it is necessary to check if the corresponding zip package for the material exists. If it exists, it indicates that the decompression process was interrupted abnormally.
            // Therefore, it is necessary to delete the file folder and the zip package and download them again.
            File tempFile = new File(directory, tempFileName);
            if (tempFile.exists()) {  // If the temporary files still exist, it indicates that the decompression of the zip file has not been completed or the renaming of other files has not been completed. Therefore, delete these temporary files.
